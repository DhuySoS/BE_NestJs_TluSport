...
async function main() {
  ...
  // 2. Hash mật khẩu mặc định: Password123@
  const defaultPassword = 'Password123@';
  const passwordHash = await bcrypt.hash(defaultPassword, 10);

  // 3. Seed 3 Tài khoản tương ứng với 3 Role
  for (const u of users) {
    const userRes = await client.query(
      `INSERT INTO users (email, password_hash, first_name, last_name, phone_number, is_active, created_at, updated_at)
       ...
       RETURNING id, email`,
      [u.email, passwordHash, u.firstName, u.lastName, u.phoneNumber],
    );

    const userId = userRes.rows[0].id;
    const roleId = roleMap.get(u.roleName);

    // Gán Role vào user_roles
    if (roleId) {
      await client.query(
    ...
    console.log(
      `✔ Tài khoản: "${u.email}" | Quyền: "${u.roleName}" | Mật khẩu: "${defaultPassword}"`,
    );
  }
